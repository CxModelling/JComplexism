...
public class PrByYear extends AbsDataFunction {

    private double[] Years;
    private double MinYear, MaxYear;


    private Map<Double, Binom> Samplers;
    private Map<Double, Double> Data;

    public PrByYear(String name, String year_col, JSONObject df) throws JSONException {
    ...
    public PrByYear(String name, JSONObject df) throws JSONException {
        this(name, "Year", df);
    }
    ...
    private Map<Double, Binom> formSamplers() {
        Map<Double, Binom> dt = new HashMap<>();
        for (Map.Entry<Double, Double> ent : Data.entrySet()) {
            dt.put(ent.getKey(), new Binom(1, ent.getValue()));
        }
        return dt;
    }
